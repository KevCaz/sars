...
sa <- function(x, a) (1 - x) ^ a
sa2 <- function(x, a) (1 - x)^(2 * a)
...
coleman <- function(data, area) {
  ...
  ## check each species has 1 or more individuals/each sites as at least one species
  ## present
  ...
  res <- list(Predicted_values = s_hat, Standard_deviation = sd, Relative_areas = ra,
    Species_richness = ob_sp)
  class(res) <- "coleman"
  res
}
